...
static int ov9734_power_on(struct device *dev)
{
	...
	/* Let MCLK stabilize before deasserting reset */
	usleep_range(1000, 1500);

	/* Deassert reset: logical 0 = physical HIGH for ACTIVE_LOW reset */
...
static const struct dev_pm_ops ov9734_pm_ops = {
	SET_RUNTIME_PM_OPS(ov9734_power_off, ov9734_power_on, NULL)
};
...
static int ov9734_probe(struct i2c_client *client)
{
	...
	/* Get reset GPIO — keep asserted (logical 1 = physical LOW for ACTIVE_LOW) */
	ov9734->reset_gpio = devm_gpiod_get_optional(ov9734->dev, "reset",
						      GPIOD_OUT_HIGH);
	if (IS_ERR(ov9734->reset_gpio)) {
		dev_warn(ov9734->dev, "failed to get reset gpio: %ld\n",
			 PTR_ERR(ov9734->reset_gpio));
